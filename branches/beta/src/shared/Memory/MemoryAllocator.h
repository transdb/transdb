...
#define USE_INTEL_SCALABLE_ALLOCATORS
...
static void *aligned_malloc(size_t size)
{
    ...
    pPtr = scalable_aligned_malloc(size, MALLOC_ALIGNMENT);
#else
    pPtr = malloc(size);
#endif
    if(pPtr == NULL)
...
{
#ifdef USE_INTEL_SCALABLE_ALLOCATORS
    scalable_aligned_free(memblock);
#else
    free(memblock);
#endif
}
...
static void *aligned_realloc(void *memblock, size_t newSize)
{
    void *pPtr;
#ifdef USE_INTEL_SCALABLE_ALLOCATORS
    pPtr = scalable_aligned_realloc(memblock, newSize, MALLOC_ALIGNMENT);
#else
...
public:
    explicit FixedPool(size_t initialSize) : m_memoryBlockSize(0), m_initialSize(initialSize)
	{
        uint8 *pMemoryBlock;
    ...
    void free(T* p)
	{
        ...
        //get memoryblock + header and calc chunk number
